                    ...
                    if (ticketCollection.getSize() == 0) {
                        ...
                        if (keyExist && user.equals(ticketUser)) {
                            ...
                            if (oldprice < Float.parseFloat(price)) {
                                ...
                                serverSender.send(clientSocket, "Указанная цена меньше,чем у данного билета,ничего не изменено.", 0);
                        } else
                            serverSender.send(clientSocket, "Билет с указанным номером отсутвует в коллекции или не принадлежит вам.", 0);
                    }
                }  else serverSender.send(clientSocket,"Элемент отсутвует в базе данных.",0);
            }
        }
